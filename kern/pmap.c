...
mem_init(void)
{
	...
	//pages initializes as a ptr to a array of Page	
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	//Page *iopmempg = pa2page((physaddr_t)IOPHYSMEM);
	//Page *extpmempg = pa2page((physaddr_t)EXTPHYSMEM);
	physaddr_t iopn = PGNUM(IOPHYSMEM);
	physaddr_t extpn = PGNUM(PADDR(ROUNDUP(pages+npages* sizeof (struct Page),PGSIZE)));
	pages[0].pp_ref = 1;
...
page_alloc(int alloc_flags)
{
	...
	//cprintf("page alloc: %x",page_free_list);
	if(page_free_list)//have free page
		page_free_list = page_free_list->pp_link;
	if(alloc_flags & ALLOC_ZERO)//need alloc zero
		if(allocPage!=NULL){
			char * page = page2kva(allocPage);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pgdir[index] & PTE_P){
		pte = (pte_t *)KADDR(PTE_ADDR(pgdir[index]));// + PTX(va);
		return pte+PTX(va); 
	}
	...
	if(create != 0){
		...
		page->pp_ref++;
		pgdir[index] = page2pa(page)|PTE_P|PTE_U|PTE_W;
		pte = (pte_t *)page2kva(page);// + PTX(va);
		return pte + PTX(va);
	}
	//non-exist & non-create
	return NULL;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	pte_t *pte;
	while(tmpsize < size){
		pte = pgdir_walk(pgdir, (void *)(va + tmpsize), 1);
		*pte = (PTE_ADDR(pa) + tmpsize)|PTE_P|perm;
		tmpsize += PGSIZE;
	}
}
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	pde_t *pte;
	while(tmpsize < size){
		//pte = pgdir_walk(pgdir, (void *)(va + tmpsize), 0);
		pte = &pgdir[PDX((void *)(va+tmpsize))];
		*pte = (pa + tmpsize)|PTE_P|PTE_PS|perm;
		tmpsize += PTSIZE;
	}
	//uint32_t cr4 = rcr4();
	//cr4 |= CR4_PSE;
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pte_t *pte;
	pp->pp_ref++;//add ref first
	page_remove(pgdir, va);//if exists, wont be 
	pte = pgdir_walk(pgdir, va, 1);
	if (pte)
		*pte = page2pa(pp) | perm| PTE_P;
	else{
